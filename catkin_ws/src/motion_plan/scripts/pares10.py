    ...
    if valor1x.get() > -9 and valor1x.get() < 9 and valor1y.get() > -9 and valor1y.get() < 9 and var==2: 
        def load_gazebo_models(block2_pose=Pose(position=Point(x=valor1x.get(), y= valor1y.get(), z=2)),
            ...
            try:
                ...
                resp_urdf = spawn_urdf("ballK11", block2_xml, "/",
                                       block2_pose, block2_reference_frame)
            except rospy.ServiceException, e:
                rospy.logerr("Spawn URDF service call failed: {0}".format(e))        

        rospy.init_node('ObjetoCreadoPK')
        load_gazebo_models()
...
def otro():
    raiz.destroy()
    os.system('rosrun motion_plan ventana11.py')
...
Ingreso.grid(row=2, column=1,pady=25)
...
botonObjeto1= Button(principal, text="Cargar Objeto 1", command=ingresovalor1x, font=("Arial", 20))
